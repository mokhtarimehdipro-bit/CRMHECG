...
function handleSignalFormerAlternant(p, ss) {
  try {
    ...
    // Vérifier qu'il n'y a pas déjà une entrée active pour ce couple (entreprise, étudiant)
    ...
    if (doublon) return createJsonResponse({ success: false, message: "Un mouvement est déjà en cours de confirmation pour cet alternant." });
    s.appendRow(["ANC-" + Date.now(), idEnt, idEtu, p.nom || "", p.prenom || "", new Date(), "À_confirmer"]);
    logAction(p.idAuteur, p.roleAuteur, "Création", "AncienAlternant", idEtu, "Mouvement signalé manuellement vers " + idEnt);
    return createJsonResponse({ success: true });
...
function handleGedScanForStudent(p, ss) {
  try {
    ...
    function match(folderName) {
      ...
      // Correspondance exacte "NOM Prénom" ou "Prénom NOM"
      var exact1 = nomNorm + " " + prenomNorm;   // dupont jean
      var exact2 = prenomNorm + " " + nomNorm;   // jean dupont
      if (fn === exact1 || fn === exact2) return true;
      // Correspondance partielle : le dossier commence par "NOM Prénom"
      if (prenomNorm && (fn.startsWith(exact1) || fn.startsWith(exact2))) return true;
      // Correspondance sans prénom (si prénom vide ou très court)
